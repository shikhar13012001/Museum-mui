...
const MoreArtifacts = (props) => {
    ...
    const FetchData=async()=>{
        const res= await fetch(`https://collectionapi.metmuseum.org/public/collection/v1/search?has_images=true&q=${artist||"vermeer"}`)
    ...
    return (
      ...
      <ImageGallery autoPlay={true} showThumbnails={false} lazyLoad={true} showNav={false} items={data} onClick={e=>{props.history.push(`/artifact/${map.get(e.target.currentSrc)}`)}}/>)
      </div>)
      :(<Box
        sx={{
          width: "100%",
